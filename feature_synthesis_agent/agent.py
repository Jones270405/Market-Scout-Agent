...
def generate_pdf(company: str, features: list, run_date: str) -> str:
    ...
    try:
        ...
        pdf_path = f"{company}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
...
def generate_briefing(company: str, features: list, run_date: str) -> str:
    ...
    Saves to disk and returns the absolute file path.
    """
...
"""
    ...
    briefing_path = f"{company}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_briefing.txt"
    with open(briefing_path, "w", encoding="utf-8") as bf:
        bf.write(briefing)

    return os.path.abspath(briefing_path)
...
feature_synthesis_agent = LlmAgent(
    ...
    instruction="""You are a Feature Synthesis Agent.
You have two tools: generate_pdf and generate_briefing.
When given a company name, feature list, and run date:
1. Call generate_pdf(company, features, run_date).
2. Call generate_briefing(company, features, run_date).
Return both file paths exactly as received.
""",
